...
func GetBlock(db DatabaseReader, hash common.Hash, number uint64) *types.Block {
	...
	return types.NewBlockWithHeader(header).WithBody(body.Transactions, body.InputTxs, body.Uncles, body.HbftStages)
}
...
func WriteChainConfig(db ethdb.Putter, hash common.Hash, cfg *params.ElephantChainConfig) error {
...
func GetChainConfig(db DatabaseReader, hash common.Hash) (*params.ElephantChainConfig, error) {
	jsonChainConfig, _ := db.Get(append(configPrefix, hash[:]...))
	if len(jsonChainConfig) == 0 {
		return nil, ErrChainConfigNotFound
	}

	var config params.ElephantChainConfig
