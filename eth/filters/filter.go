...
// JiangHan: Logs函数搜索并匹配所有的日志，将所有匹配的block都返回，并更新这个filter的进度
...
// JiangHan: 基于BloomBits过滤的日志匹配查询机制（结果可以来自本地fullnode,或者来自网络les/odr）
...
func (f *Filter) indexedLogs(ctx context.Context, end uint64) ([]*types.Log, error) {
	...
	// jh: 从 f.begin 到 end 的范围进行查询，将结果发送到 matches 通道
	...
	for {
		...
		case number, ok := <-matches:
			...
			// jh: 获取到了匹配的 block header number 编号，于是获取这个 header 并检查它
