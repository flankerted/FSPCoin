...
func (p *FakePeer) RequestBodies(hashes []common.Hash) error {
	...
	for _, hash := range hashes {
		...
		nextSealers = append(nextSealers, block.NextSealers())
		nextPolice = append(nextPolice, block.NextPolice())
		validHeights = append(validHeights, block.GetValidHeadersSlice())
	}
	p.dl.DeliverBodies(p.id, txs, uncles, nextSealers, nextPolice, validHeights)
	return nil
}
