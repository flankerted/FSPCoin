...
func (q *queue) Results(block bool) []*fetchResult {
	...
	if len(results) > 0 {
		...
		for _, result := range results {
			...
			for _, sealer := range result.NextSealers {
				size += common.StorageSize(len(sealer.Address) + len(sealer.NodeStr))
			}
			for _, officer := range result.NextPolice {
				size += common.StorageSize(len(officer.Address) + len(officer.NodeStr))
			}
			for _, validHeight := range result.ValidHeights {
				size += validHeight.Size()
			}
...
func (q *queue) ReserveBodies(p *peerConnection, count int) (*fetchRequest, bool, error) {
	isNoop := func(header *types.Header) bool {
		return header.TxHash == types.EmptyRootHash && header.UncleHash == types.EmptyUncleHash &&
			header.EleSealersHash == types.EmptyElectorHash && header.PoliceHash == types.EmptyElectorHash &&
			header.ValidHeightHash == types.EmptyValidHeightHash
	}
	q.lock.Lock()
	defer q.lock.Unlock()
...
func (q *queue) DeliverBodies(id string, txLists [][]*types.Transaction, uncleLists [][]*types.Header,
	sealerLists, policeLists [][]common.AddressNode, validHeightsLists [][]*types.VerifiedValid) (int, error) {
	q.lock.Lock()
	defer q.lock.Unlock()

	reconstruct := func(header *types.Header, index int, result *fetchResult) error {
		if types.DeriveSha(types.Transactions(txLists[index])) != header.TxHash || types.CalcUncleHash(uncleLists[index]) != header.UncleHash ||
			types.DeriveSha(types.NextEleSealersList(sealerLists[index])) != header.EleSealersHash ||
			types.DeriveSha(types.NextPoliceList(policeLists[index])) != header.PoliceHash ||
			types.DeriveSha(types.VerifiedValids(validHeightsLists[index])) != header.ValidHeightHash {
			return errInvalidBody
		}
		result.Transactions = txLists[index]
		result.Uncles = uncleLists[index]
		result.NextSealers = sealerLists[index]
		result.NextPolice = policeLists[index]
		result.ValidHeights = validHeightsLists[index]
		return nil
	}
