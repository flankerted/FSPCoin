...
func (self *StateDB) HandleElection(addr common.Address) {
	// Check balance
	if !common.GetConsensusBftTest() {
		...
		if leftBalance.Cmp(electionNeedBalance) < 0 {
			log.Error("Add to election pool", "left balance", leftBalance, "election need balance", electionNeedBalance)
			return
		}

		// Add election for the address
		self.SubBalance(addr, electionNeedBalance)
	}
}
...
func (self *StateDB) ActOperate(addr common.Address, actType uint8, actData []byte) {
	switch actType {
	case baseparam.TypeSealersElection:
		self.HandleElection(addr)
	//case baseparam.TypeValidateElection:
	//	self.ValidateElection()
	default:
	}
}
