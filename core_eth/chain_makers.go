...
// JiangHan：（重点)交易增加进 Tx 列表，同时调用 ApplyTransaction 执行交易
// 如果其中有智能合约，也同时执行
...
func (b *BlockGen) OffsetTime(Milliseconds int64) {
	...
	b.header.Difficulty = b.engine.CalcDifficulty(b.chainReader, b.header.Time.Uint64()/1000, b.parent.Header())
}
...
func makeHeader(chain consensus.ChainReader, parent *types.Block, state *state.StateDB, engine consensus.Engine) *types.Header {
	...
	if parent.Time() == nil {
		...
		time = new(big.Int).Add(parent.Time(), big.NewInt(10*1000)) // block time is fixed at 10 seconds
	}
	...
	return &types.Header{
		...
		Difficulty: engine.CalcDifficulty(chain, time.Uint64()/1000, &types.Header{
			Number:     parent.Number(),
			Time:       new(big.Int).Sub(time, big.NewInt(10*1000)),
