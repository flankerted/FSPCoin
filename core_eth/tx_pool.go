...
func (pool *TxPool) validateTx(tx *types.Transaction, local bool) error {
	...
	// cost == V + GP * GL
	if pool.currentState.GetBalance(from).Cmp(tx.Cost()) < 0 && !(common.GetConsensusBftTest() && from.Equal(*tx.To())) {
...
func (pool *TxPool) promoteExecutables(accounts []common.Address) {
	...
	for _, addr := range accounts {
		...
		drops, _ := list.Filter(addr, pool.currentState.GetBalance(addr), pool.currentMaxGas)
...
func (pool *TxPool) demoteUnexecutables() {
	...
	for addr, list := range pool.pending {
		...
		drops, invalids := list.Filter(addr, pool.currentState.GetBalance(addr), pool.currentMaxGas)
		for _, tx := range drops {
			hash := tx.Hash()
