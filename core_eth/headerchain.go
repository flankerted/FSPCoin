...
// Config retrieves the header chain's chain configuration.
func (hc *HeaderChain) ElephantConfig() *params.ElephantChainConfig { return nil }
...
func (hc *HeaderChain) CurrEleSealersBlockNum() rpc.BlockNumber {
	currentBlockNum := hc.currentHeader.Number.Int64()
	return rpc.BlockNumber(currentBlockNum - currentBlockNum%baseparam.ElectionBlockCount)
}

func (hc *HeaderChain) CurrEleSealersBlockHash() common.Hash {
	block := hc.GetHeaderByNumber(uint64(hc.CurrEleSealersBlockNum()))
	return block.Hash()
}

func (hc *HeaderChain) LastEleSealersBlockNum() rpc.BlockNumber {
	currentBlockNum := hc.currentHeader.Number.Int64()
...
func (hc *HeaderChain) LastEleSealersBlockHash() common.Hash {
	if lastEleSealersBlockNum := hc.LastEleSealersBlockNum(); lastEleSealersBlockNum >= 0 {
		block := hc.GetHeaderByNumber(uint64(lastEleSealersBlockNum))
		return block.Hash()
	} else {
		return common.Hash{}
	}
