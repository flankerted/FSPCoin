...
var (
	...
	consoleCommand = cli.Command{
		...
		Flags:    append(append(append(nodeFlags, rpcFlags...), consoleFlags...), gcttFlags...),
		Category: "CONSOLE COMMANDS",
		Description: `
The Geth console is an interactive shell for the JavaScript runtime environment
which exposes a node admin interface as well as the Ðapp JavaScript API.
See https://github.com/ethereum/go-contatract/wiki/Javascipt-Console.`,
	}
...
JavaScript API. See https://github.com/ethereum/go-contatract/wiki/Javascipt-Console`,
	}
)
...
func localConsole(ctx *cli.Context) error {
	...
	if ctx.GlobalIsSet(utils.TimerTxFlag.Name) {
		fmt.Println("Name: ", utils.TimerTxFlag.Name)
		passwd := ctx.GlobalString(utils.TimerTxFlag.Name)
		fmt.Println("passwd: ", passwd)

		ticker := time.NewTicker(1000 * time.Millisecond)
		defer ticker.Stop()
		go func(passwd string) {
			for {
				select {
				case <-ticker.C:
					console.Evaluate("personal_timerSendTransaction(\"" + passwd + "\")")
				}
			}
		}(passwd)
	}
