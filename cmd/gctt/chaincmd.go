...
func initGenesis(ctx *cli.Context) error {
	...
	for _, name := range []string{"chaindata", "lightchaindata"} {
		...
		_, hash, err := core_eth.SetupGenesisBlock(chaindb, genesis)
		if err != nil {
...
func copyDb(ctx *cli.Context) error {
	...
	hc, err := core_eth.NewHeaderChain(db, chain.Config(), chain.Engine(), func() bool { return false })
	if err != nil {
		return err
	}
...
func dump(ctx *cli.Context) error {
	...
	for _, arg := range ctx.Args() {
		...
		if block == nil {
			...
			state, err := state_eth.New(block.Root(), state_eth.NewDatabase(chainDb))
			if err != nil {
