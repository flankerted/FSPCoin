...
func fill(src []byte, size int) []byte {
	m := len(src) % size
	if m == 0 {
		m = size
	} else {
...
func Encrypt(in, key []byte, final bool) ([]byte, error) {
	keySize := 8
	if len(key) < keySize {
		key = fill(key, keySize)
	} else {
		key = key[:keySize]
	}
	...
	if !final && len(in)%blockSize != 0 {
		return nil, errors.New("Data len is not right")
	}
	if final {
		in = fill(in, blockSize)
	}
...
func Decrypt(in, key []byte, final bool) ([]byte, error) {
	keySize := 8
	if len(key) < keySize {
		key = fill(key, keySize)
	} else {
