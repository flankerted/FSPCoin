...
func (w *ledgerDriver) SignTx(path accounts.DerivationPath, tx interface{}, chainID *big.Int) (common.Address, interface{}, error) {
	...
	if value, ok := tx.(*types.Transaction); ok {
		return w.ledgerSign(path, value, chainID)
	}
