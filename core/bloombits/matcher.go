...
import (
	...
	"github.com/contatract/go-contatract/common/bitutil"
...
// JiangHan: Matcher 是一个对bit流做 and/or 操作的管道系统，生成一个为了找到指定数据内容而
// 可能需要加载的block的stream。（为什么说是可能？因为bloombit是一个有一定误差的查找算法，所以
// 我们根据某些位加载回来的不一定都是我们需要的block）
...
func (s *MatcherSession) Multiplex(batch int, wait time.Duration, mux chan chan *Retrieval) {
	for {
		...
		case mux <- request:
			// jh: 发送一个request(chan) 给 mux,
			// eth/bloombits.go（或者les/bloombits.go） 中的 startBloomHandlers() 中的
			// case request := <-eth.bloomRequests: 就能触发，相当于将一个命令通道传递给了
			// 对方，然后我们这里接着往这个request(chan)命令通道发送一个
			// &Retrieval{Bit: bit, Sections: sections, Context: s.ctx}
			// 对方的task := <-request就能接到这个对象，当做它的task
			// Retrieval accepted, something must arrive before we're aborting
			request <- &Retrieval{Bit: bit, Sections: sections, Context: s.ctx}

			// 对方干活,我们在这里等待从这个request(chan)中得到对方对这个task的执行回复
			result := <-request
