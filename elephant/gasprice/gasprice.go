...
func NewOracle(backend elephantapi.Backend, params Config) *Oracle {
...
func (gpo *Oracle) getBlockPrices(ctx context.Context, signer types.Signer, blockNum uint64, ch chan getBlockPricesResult) {
	...
	// JiangHan:
	// elephant 没有oracle预测，价格可以按照 eth 的最近块来推测
	...
	for _, tx := range txs {
		/*
		sender, err := types_eth.Sender(signer, tx)
