...
type fetchResult struct {
	...
	InputTxs     types.ShardingTxBatches
	Receipts     types.Receipts
	HbftStages   types.HBFTStageCompletedChain
}
...
func (q *queue) Results(block bool) []*fetchResult {
	...
	if len(results) > 0 {
		...
		for _, result := range results {
			...
			for _, inputTx := range result.InputTxs {
				size += inputTx.Size()
			}
			for _, stage := range result.HbftStages {
				size += stage.Size()
			}
...
func (q *queue) ReserveBodies(p *peerConnection, count int) (*fetchRequest, bool, error) {
	isNoop := func(header *types.Header) bool {
		//return header.TxHash == types.EmptyRootHash && header.UncleHash == types.EmptyUncleHash && header.BftHash == types.EmptyHbftHash
		return header.TxHash == types.EmptyRootHash && header.BftHash == types.EmptyHbftHash
	}
	q.lock.Lock()
	defer q.lock.Unlock()
...
func (q *queue) DeliverBodies(id string, txLists [][]*types.Transaction, inputTxLists [][]*types.ShardingTxBatch, uncleLists [][]*types.Header,
	hbftStagesLists [][]*types.HBFTStageCompleted) (int, error) {
	q.lock.Lock()
	defer q.lock.Unlock()
	...
	reconstruct := func(header *types.Header, index int, result *fetchResult) error {
		...
		if types.DeriveSha(types.ShardingTxBatches(inputTxLists[index])) != header.InputTxHash {
			return errInvalidBody
		}
		...
		result.InputTxs = inputTxLists[index]
		result.Uncles = uncleLists[index]
		result.HbftStages = hbftStagesLists[index]
		return nil
	}
