...
func SetProtocolMinVer(minver, maxver int) {
	protocolMinVer = minver
	protocolMaxVer = maxver
}
...
type LightPeer interface {
	Head() (common.Hash, int64, int64, common.Hash, *big.Int)
...
func (w *lightPeerWrapper) Head() (common.Hash, int64, int64, common.Hash, *big.Int) {
	return w.peer.Head()
}
...
func newPeerConnection(id string, version int, peer Peer, logger log.Logger) *peerConnection {
	//log.Info("[NEW PEER CONNECTION]", "id", id)
	//debug.PrintStack()
...
func (p *peerConnection) FetchHeaders(from uint64, count int) error {
	...
	if p.version < protocolMinVer {
		panic(fmt.Sprintf("header fetch [eth/%d+] requested on eth/%d", protocolMinVer, p.version))
	}
...
func (p *peerConnection) FetchBodies(request *fetchRequest) error {
	// Sanity check the protocol version
	if p.version < protocolMinVer {
		panic(fmt.Sprintf("body fetch [eth/%d+] requested on eth/%d", protocolMinVer, p.version))
	}
	// Short circuit if the peer is already fetching
...
func (p *peerConnection) FetchNodeData(hashes []common.Hash) error {
	// Sanity check the protocol version
	if p.version < protocolMinVer {
		panic(fmt.Sprintf("node data fetch [eth/%d+] requested on eth/%d", protocolMinVer, p.version))
	}
	// Short circuit if the peer is already fetching
...
func (ps *peerSet) HeaderIdlePeers() ([]*peerConnection, int) {
	...
	return ps.idlePeers(protocolMinVer, protocolMaxVer, idle, throughput)
}
