...
func (c *Clique) verifyHeader(chain consensus.ChainReader, header *types.Header, parents []*types.Header) error {
	...
	if header.Time.Cmp(big.NewInt(time.Now().UnixNano()/1e6)) > 0 {
...
func (c *Clique) verifyCascadingFields(chain consensus.ChainReader, header *types.Header, parents []*types.Header) error {
	...
	if parent.Time.Uint64()+c.config.Period*1000 > header.Time.Uint64() {
...
func (c *Clique) Prepare(chain consensus.ChainReader, header *types.Header) error {
	...
	header.Time = new(big.Int).Add(parent.Time, new(big.Int).SetUint64(c.config.Period*1000))
...
func (c *Clique) Seal(chain consensus.ChainReader, block *types.Block, stop <-chan struct{}) (*types.Block, error) {
	...
	delay := time.Unix(header.Time.Int64()/1000, 0).Sub(time.Now()) // nolint: gosimple
